...
import { check, drop, exportRaw, generate, generateFromJournalCommand, migrate, pull, push, studio, up } from './schema';
import { ormCoreVersions } from './utils';

const version = async () => {
	const { npmVersion } = await ormCoreVersions();
	const ormVersion = npmVersion ? `drizzle-orm: v${npmVersion}` : '';
	const envVersion = process.env.WIZZLE_VERSION || process.env.DRIZZLE_KIT_VERSION;
	const kitVersion = envVersion ? `v${envVersion}` : '--';
	const versions = `wizzle: ${kitVersion}\n${ormVersion}`;
...
run([generate, migrate, pull, push, studio, up, check, drop, generateFromJournalCommand, exportRaw, ...legacy], {
	name: 'wizzle',
	version: version,
});
